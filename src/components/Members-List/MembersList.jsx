...
const MembersList = () => {
  ...
  const tasks = useSelector((state) => state.taskDetails.tasks);
  console.log(tasks, "tasksss");
  const handleCheckboxChange = (member) => {
    if (selectedMember === member) {
      setSelectedMember(null);
    } else {
      setSelectedMember(member.profilePicture);
    }
  };
  const members = [
    {
          ...
          <table className="table-auto mx-auto w-[55%] text-black">
            {/* <tbody>
              {members?.map((member, index) => (
                <tr className="focus-within:bg-black">
                  <td
                    key={index}
                    className="px-6 py-6 text-xl flex items-center gap-3"
                  >
                    <img
                    ...
                    <td className="px-6 py-6 text-xl">
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          // onClick={() =>
                          //   dispatch(addTaskCard(member.profilePicture))
                          // }
